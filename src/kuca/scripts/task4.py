...
class MoveItDemo:
    ...
    #    rospy.sleep(2)
        ...
        target_pose.pose.position.x -= 0.215 
        ...
        while count < 2:
            ...
            target = right_arm.get_current_pose()
            q = quaternion_from_euler(0, -1, 0)
            target.header.frame_id = 'base_footprint'
            #target.pose.position.x += 0.025 * (count - 1)
            ...
            #right_arm.set_joint_value_target(target, end_effector_link, True)
            right_arm.set_pose_target(target)

            right_arm.go()
            rospy.sleep(1)
            
            self.move_arm(right_arm, 4)
