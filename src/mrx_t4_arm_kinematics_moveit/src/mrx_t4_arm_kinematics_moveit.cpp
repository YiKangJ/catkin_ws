#include <mrx_t4_arm_kinematics_moveit/mrx_t4_arm_kinematics_moveit.h>
#include <mrx_t4_arm_kinematics_moveit/configuration_comparator.h>
...
//CLASS_LOADER_REGISTER_CLASS(mrx_t4_arm_kinematics_moveit::KinematicsPlugin,
//        kinematics::KinematicsBase);

PLUGINLIB_EXPORT_CLASS(mrx_t4_arm_kinematics_moveit::KinematicsPlugin,
        kinematics::KinematicsBase);

using namespace mrx_t4_arm_kinematics_moveit;


KinematicsPlugin::KinematicsPlugin() : NUM_JOINTS(4)
{
}
        ...
        double search_discretization)
{
    ...
    ik_.reset(new mrx_t4_arm_kinematics::InverseKinematics(lower_limits,
...
{   
	...
	const KDL::Frame frame_ = frame;

    // Convert the seed array to a KDL joint array
    const KDL::JntArray seed = configurationStdToKdl(ik_seed_state);

    // Calculate the inverse position kinematics
    std::vector<KDL::JntArray> kdl_solutions;
	
    int res = ik_->CartToJnt(seed, frame_, kdl_solutions);
